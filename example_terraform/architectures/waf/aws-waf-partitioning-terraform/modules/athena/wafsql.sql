...
WITH dataset AS (
  SELECT
    ...
    httprequest.uri AS path,
    header,
    action,
    t.rulegroupid AS rulegroupid,
    t.terminatingrule.ruleid AS TerminatingRuleID,
    t.nonterminatingmatchingrules AS NonTerminatingRules
  FROM "waf_logs" 
  CROSS JOIN UNNEST(rulegrouplist) AS t(t)
  CROSS JOIN UNNEST (httprequest.headers) AS s(header)
  ...
  GROUP BY action,t.rulegroupid, httprequest.uri, s.header,t.terminatingrule.ruleid, t.nonterminatingmatchingrules
)
SELECT 
  count,
  header.value,
...
GROUP BY count, header.value, path, action
ORDER BY count DESC
...
WITH dataset AS (
  ...
  WHERE (date = '2024/02/01' or date = '2024/02/02' or date = '2024/02/03' or date = '2024/02/04' 
    ...
    AND (t.terminatingrule.ruleid is not null 
    OR CARDINALITY(t.nonterminatingmatchingrules) != 0)
  GROUP BY action,t.rulegroupid, s.header,t.terminatingrule.ruleid, t.nonterminatingmatchingrules
)
SELECT 
  count,
  header.value,
...
AND (t.terminatingrule.ruleid is not null 
OR CARDINALITY(t.nonterminatingmatchingrules) != 0)
GROUP BY t.rulegroupid,httprequest.uri, t.terminatingrule.ruleid, t.nonterminatingmatchingrules
ORDER BY "count" DESC 
...
AND (t.terminatingrule.ruleid is not null 
OR CARDINALITY(t.nonterminatingmatchingrules) != 0)
GROUP BY DATE_TRUNC('hour',from_unixtime(timestamp/1000) AT TIME ZONE 'Asia/Seoul'), action, httpsourceid, httprequest.clientip, t.rulegroupid, t.terminatingrule.ruleid, t.nonterminatingmatchingrules
